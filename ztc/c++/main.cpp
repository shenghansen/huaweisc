...
//根据要前进的距离获取线速度
float get_line_speed(float distance){
    if(distance>0){
...
void go_to_workstation(int robot_id,int workstation_id){
    ...
    order(FORWARD,robot_id,get_line_speed(distance)).printf_order();
}


bool readUntilOK() {
    bool first=false;
    fout=ofstream("/home/asc2022team0/huaweisc/ztc/c++/log.txt",ios::app);
    if (!map_ok){
        char line[1024]={'*'};
...
int main() {
    remove("/home/asc2022team0/huaweisc/ztc/c++/log.txt");
    readUntilOK();
    puts("OK");
    fflush(stdout);

    /* 
        robot_buyed: 记录四个机器人的装载状态
        robot_target: 下一个前往购买目标的工作站的id
        robot_target_type: 下一个要前往购买的物品的类型
    */
    bool robot_buyed[4] = {0};
    int robot_target[4];
    int robot_target_type[4];
    memset(robot_target, -1, sizeof(robot_target));
    memset(robot_target_type, -1, sizeof(robot_target_type));
    ...
    while (scanf("%d", &frameID) != EOF) {
        ...
        */
        ...
        /* ROBOT 1 */
        if (!robot_buyed[0]) {  // 状态：空载
            if (robot_target[0] == -1) {   // 状态：空载-空闲
                // 动作1：购买 4
                int buy_from = -1;
                for (int i=0; i<workstation_num; i++) {
                    if (workstation_type[i]==4 && product_status[i]==1) { // 找到一个生产好的4号产品
                        buy_from = i;
                        break;
                    }
                }
                if (buy_from != -1) {
                    robot_target[0] = buy_from;
                    go_to_workstation(0, robot_target[0]);
                }
                // 动作2：购买 1
                if (buy_from != -1) {
                    for (int i=0; i<workstation_num; i++) {
                        if (workstation_type[i]==1 && product_status[i]==1) { // 找到一个生产好的1号产品
                            buy_from = i;
                            break;
                        }
                    }
                ...
                // 产能过剩，总能找到一个生产好的物品去购买
            } else {    // 状态：空载-继续前进购买
                if (robot_target_type[0] == 4) {    
                    int sell_to = -1;
                    for (int i=0; i<workstation_num; i++) {
                        if (workstation_type[i]==4 && (raw_material_status[i]^(1<<1)))
                    }
                } else if (robot_target_type[0] == 1) {
